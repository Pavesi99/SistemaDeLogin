...
if(isset($_GET['email']) && isset($_GET['token'])) {
   ...
   }else {#e-mail ou token Invalidos
        header("location:index.php");
        exit();
   }
   
}else{ #Digitou a pagina sem email e token
    header("location:index.php");
     exit();
}
      ...
      <main class="container">
          <section class="row justify-content-center">
              <div class= "col-lg-5 mt-5" >
                ...
                <?= @$msg ?> <!-- Essa forma so aceita um valor igual o if sem {}-->
                  </h4>
                  <form method="post"> <!-- Envio para a própria pagina -->
                      <div class="form-group">
                                <label for="senha">Nova SENHA</label>
                      <input type="password" name="senha" id="senha" class="form-control" placeholder="Nova SENHA" required>
                      </div>
                
                      <div class="form-group"> <label for="csenha">Confirmar SENHA</label>
